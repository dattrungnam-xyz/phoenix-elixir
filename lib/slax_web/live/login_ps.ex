defmodule SlaxWeb.LoginPasswordLive do
  alias Slax.Login
  alias Slax.Accounts
  ...
  def handle_event("login_email", params, socket) do
    ...
    IO.inspect(params)
    %{"user" => user} = params
    changeset = Accounts.change_user_login(%User{}, user)
  ...
  def mount(_params, _session, socket) do
    ...
    form = to_form(%{"user" => email}, as: "user")
    {:ok, assign(socket, form: form), temporary_assigns: [form: form]}
  end
        ...
        <div>
          <div class="flex flex-1 w-[360px] max-w-[360px] h-fit gap-[25px] flex-col rounded-xl bg-[white] items-center">
            ...
            <.simple_form
              for={@form}
              id="login_form"
              phx-change="login_email"
              action={~p"/users/log_in"}
            >
              <.input disabled field={@form[:email]} type="text" label="Email or username" required></.input>
