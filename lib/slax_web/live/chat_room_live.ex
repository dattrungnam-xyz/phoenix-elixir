defmodule SlaxWeb.ChatRoomLive do
  ...
  def handle_event("toggle-topic", _params, socket) do
    {:noreply, assign(socket, hideTopic?: !socket.assigns.hideTopic?)}
  end
  ...
  def render(assigns) do
    ...
    <.modal
      ...
      on_cancel={JS.navigate(~p"/room/#{@room}")}
    >
  ...
  def handle_event("submit-message", %{"message" => message_param}, socket) do
    %{current_user: current_user, room: room} = socket.assigns

    socket =
      if Chat.joined?(room, current_user) do
        case Chat.create_message(room, message_param, current_user) do
          {:ok, _message} ->
